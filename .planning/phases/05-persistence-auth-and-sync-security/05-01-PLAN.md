...
must_haves:
  truths:
    - "Convex project is configured with Email, Google, and GitHub auth providers"
    - "Decisions schema exists with userId field for ownership"
    - "Backend functions enforce userId ownership checks"
    - "Convex types are generated and type-safe"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    6. Set data location to Europe in Convex dashboard (user will need to do this manually)

    IMPORTANT: Do NOT run `npx convex dev` or `convex dev` in this task. Just install dependencies and create files. The user will need to set up the Convex project and providers manually via the dashboard before running dev.
  </action>
  <verify>
