...
must_haves:
  truths:
    - "User can enter a required decision title and optional decision metadata (description, icon string) with icon preview."
...
<task type="auto">
  ...
  <action>Implement decision setup with required `title` and optional `description`/`icon` fields using Zod-backed validation for step gating. Treat icon as a simple string value and render an inline preview when present (no picker UI in this phase). Surface inline validation message when title is empty and keep optional fields non-blocking. Use RHF only if form complexity grows; controlled inputs are acceptable here. Bind initial values from draft state so revisit behavior repopulates fields immediately.</action>
...
<task type="auto">
  ...
  <files>src/features/decision/state/decisionPrereq.ts, src/routes/setup/DecisionSetupRoute.tsx, src/features/decision/setup/DecisionSetupForm.tsx, src/features/decision/setup/decision.schema.ts</files>
  <action>On valid submission, dispatch decision draft updates and navigate to options step. Implement `isDecisionStepComplete` helper used by router guards so forward routes rely on the same completion criteria as the form submit path, and make this helper delegate to the shared Zod decision schema (`safeParse`) to satisfy locked step-gate rules. Do not introduce ad hoc route checks inside unrelated components.</action>
