...
must_haves:
  truths:
    - "User can add, edit, delete, and reorder options with stable identity and optional metadata."
...
<task type="auto">
  ...
  <action>Create options step controls to add, edit, delete, and reorder options while keeping stable ids and dense order from domain utilities. Support optional description/icon per option and render from canonical draft state (no local shadow list) so navigation away and back preserves data. Keep destructive actions explicitly labeled.
  </action>
...
<task type="auto">
  ...
  <action>Use shared `hasMinimumOptions` helper to block continuation from options step and to protect criteria, ratings, and results routes against deep-link bypass. Show clear actionable copy when blocked (for example, "Add at least 2 options to continue") and allow access immediately once threshold is met.</action>
  <verify>With fewer than two options, attempt navigation to `#/criteria`, `#/ratings`, and `#/results` and confirm redirects/blocking; with two or more options, confirm these routes become accessible.</verify>
